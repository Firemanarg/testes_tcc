...
# [PAREI AQUI]: Estou tentando obter a porcentagem de uso da CPU usando Python, mas está lagando muito.
# Tentar usar multithread com o método OS.create_process(...) (funciona tipo o fork).
# Verificar se existe um tipo de pipe na Godot para comunicar com o novo processo.
#func _get_cpu_usage_percent() -> float:
	#var cpu_usage: float = 0.0
	#var pid: int = OS.get_process_id()
	#print("Passing pid ", pid)
	#var path: StringName = "python"
	#var args: Array = [
		#"C:\\Users\\firem\\Documents\\PUC\\Artigo JCPoli\\testes\\get_cpu_usage.py",
		#str(pid)
	#]
	#var output: Array = []
	#var new_pid: int = OS.create_process(path, args, true)
	#OS.kill(new_pid) # DEBUG
	#var exit_code: int = OS.execute(path, args, output)
	#if exit_code == 0: # Success
		#cpu_usage = int(output[0])
